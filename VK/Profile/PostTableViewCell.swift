...
class PostTableViewCell: UITableViewCell {
    ...
    func setup(for model: Post){
        ...
        likePostLabel.text = NSLocalizedString(LocalizedKeys.keyLikes, comment: "") + ": \(model.likes)"
        viewPostLabel.text = NSLocalizedString(LocalizedKeys.keyViews, comment: "") + ": \(model.views)"
    }
    ...
    func setupForFavoriteFromCoreData(for postID: String){
        ...
        if let index = coreDataManager.posts.firstIndex(where: { $0.id == postID })  {
            ...
            likePostLabel.text = NSLocalizedString(LocalizedKeys.keyLikes, comment: "") +  ": \(coreDataManager.posts[index].likes)"
            viewPostLabel.text = NSLocalizedString(LocalizedKeys.keyViews, comment: "") + ": \(coreDataManager.posts[index].view)"
        }
    }
    ...
    @objc func doubleTapFunc() {
        if index != nil {
            ...
            if let _ = coreDataManager.posts.firstIndex(where: { $0.id == model[index!.row].uniqID }) {
                print( NSLocalizedString(LocalizedKeys.keyFavoriteNoteExsist, comment: "") + " " + NSLocalizedString(LocalizedKeys.keyFavoriteNoteExsistText, comment: "") )
                coreDataManager.reloadData()
            } else {
